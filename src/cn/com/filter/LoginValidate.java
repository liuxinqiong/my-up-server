...
public class LoginValidate implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		String requestPath = req.getServletPath();//action/mapping
		// 查询字符串 ?后边的内容
		String requestQuery = req.getQueryString();
		//String path = req.getContextPath();//项目名称
		//得到绝对路径
		//String basePath = req.getScheme() + "://" + req.getServerName() + ":"+ req.getServerPort() + path + "/";//action之前的全路径
		// 检查需要登录验证
		if (checkFile(requestPath, requestQuery)) {
			...
			if (loginUser != null) {
				...
				req.getRequestDispatcher("/userAction/noLogin").forward(req, resp);
			}
		} else {
			// 不需要验证登录
			chain.doFilter(req, resp);
